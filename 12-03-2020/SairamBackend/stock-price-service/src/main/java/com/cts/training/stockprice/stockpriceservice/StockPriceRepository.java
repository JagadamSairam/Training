...
@Query("SELECT new com.cts.training.stockprice.stockpriceservice.StockModel(companyCode,stockExchange,date,AVG(currentPrice)) " + " FROM StockPriceEntity" + "WHERE companyCode=?1 AND stockExchange=?2 AND BETWEEN ?3 AND ?4 " + "GROUP BY date" )
public List<StockModel> getStockPriceBetweenDates(String companyCode, String stockExchange, LocalDate startDate, LocalDate endDate);
